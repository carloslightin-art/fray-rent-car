...
const LOCAL_VEHICLE_FALLBACK_SET = new Set(LOCAL_VEHICLE_FALLBACKS)
...
export function getVehicleGalleryUrls(vehicle = {}, maximum = 3) {
  const gallery = Array.isArray(vehicle.gallery_images) ? vehicle.gallery_images : []
  const rawImages = [vehicle.image || vehicle.image_url, ...gallery].filter(Boolean)
  const images = rawImages.map((url) => getVehicleImageUrl(url, vehicle.id))

  const realImages = Array.from(new Set(images.filter((url) => !LOCAL_VEHICLE_FALLBACK_SET.has(url))))
  if (realImages.length > 0) {
    return realImages.slice(0, maximum)
  }

  const fallbackImage = images.find((url) => LOCAL_VEHICLE_FALLBACK_SET.has(url))
  return [fallbackImage || getFallbackVehicleImageUrl(vehicle.id)]
}
